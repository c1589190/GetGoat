package com.cna.getgoat.map;

import com.cna.getgoat.config.ConfigsManager;
import com.cna.getgoat.map.data.AdminDivisionLoader;
import com.cna.getgoat.map.data.DataPathConfig;
import com.cna.getgoat.map.data.GeoJsonWriter;
...
import com.cna.getgoat.map.terrain.TerrainCache;
import com.cna.getgoat.map.terrain.TerrainGenerator;
import com.cna.getgoat.map.terrain.TerrainOverrideStore;
...
public class MapManager {
    ...
    public MapManager() {
        ...
        this.reliefElevThreshold = ConfigsManager.getReliefElevThreshold();
        this.reliefRoughThreshold = ConfigsManager.getReliefRoughThreshold();
    }
    ...
    public void setWorkspaceForOverrides(java.nio.file.Path workspaceDir) {
        if (terrainGrid != null) {
            ...
            overrideStore.setWorkspace(workspaceDir,
                com.cna.getgoat.config.ConfigsManager.getGridCellSize());
        }
    }
    ...
    public String debugClassificationPipeline(double lat, double lng) {
        ...
        TerrainType base = com.cna.getgoat.map.terrain.ReliefMapLoader.elevationFromRgb(rgb) > 0 ? com.cna.getgoat.map.terrain.TerrainType.PLAINS : com.cna.getgoat.map.terrain.TerrainType.OCEAN;
        double elev = com.cna.getgoat.map.terrain.ReliefMapLoader.elevationFromRgb(rgb);
    ...
    public String applyUpgradeRules() {
        ...
        try {
            var rl = new com.cna.getgoat.map.terrain.ReliefMapLoader();
    ...
    public String setRoughnessThreshold(double threshold) {
        ...
        for (int row = 0; row < terrainGrid.getRows(); row++) {
            for (int col = 0; col < terrainGrid.getCols(); col++) {
                ...
                cell.setElevationMeters(com.cna.getgoat.map.terrain.ReliefMapLoader.elevationFromRgb(rgb));
    ...
    public String exportTerrainCellsInRadius(double lat, double lng, double radiusKm, double displayRes) {
        checkInitialized();
        long t0 = System.currentTimeMillis();
        double nativeRes = com.cna.getgoat.config.ConfigsManager.getDisplayFineRes();
