package com.cna.getgoat.config;
...
public class CommanderConfig {
    ...
    public static CommanderConfig load(Path configFile) throws IOException {
        ...
        LlmConfig c = cfg.llm;
        if (c.provider == null || c.provider.isEmpty()) c.provider = ConfigsManager.getLlmProvider();
        if (c.model == null || c.model.isEmpty()) c.model = ConfigsManager.getLlmModel();
        if (c.endpoint == null || c.endpoint.isEmpty()) c.endpoint = ConfigsManager.getLlmEndpoint();
        if (c.apiKey == null || c.apiKey.isEmpty()) c.apiKey = ConfigsManager.getLlmApiKey();
        ...
        if (c.maxTokens <= 0) c.maxTokens = ConfigsManager.getLlmMaxTokens();
