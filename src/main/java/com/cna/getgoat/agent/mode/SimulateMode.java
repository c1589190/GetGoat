...
import com.cna.getgoat.agent.sim.*;
...
public class SimulateMode implements AgentMode {
    ...
    private void initResolvers(AbstractAgent agent) {
    ...
    private String getAllDeployments(AbstractAgent agent, String treeId, String nodeId) {
        var tree = agent.getNodesManager().getTree(treeId);
    ...
    private String resolveBattles(AbstractAgent agent, String pairsJson) {
        try {
    ...
    private String saveOutcome(AbstractAgent agent, String treeId, String parentNodeId,
                                String name, int round, String outcome) {
        try {
            var node = agent.getNodesManager().addRound(treeId, parentNodeId, name,
    ...
    private String setSideIntel(AbstractAgent agent, String side, String entriesJson) {
        try {
    ...
    private String setExploredRegion(AbstractAgent agent, String side,
    ...
    private String setPhantom(AbstractAgent agent, String targetSide,
    ...
    private void autoGenerateIntel(AbstractAgent agent, String treeId, String nodeId, int newRound) {
    ...
    private void flushIntelToNode(AbstractAgent agent, String treeId, String nodeId) {
        var tree = agent.getNodesManager().getTree(treeId);
        if (tree == null) return;
        var node = tree.findNode(nodeId);
        if (node == null) return;
     ...
     */
    public SimulationResult simulateDeterministic(AbstractAgent agent, String treeId, String nodeId) {
        initResolvers(agent);
        ...
        var newNode = agent.getNodesManager().addRound(treeId, nodeId,
