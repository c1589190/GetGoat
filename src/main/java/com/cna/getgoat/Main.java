...
import com.cna.getgoat.agent.mode.SimulateMode;
 ...
 *
 *   java -cp ... com.cna.getgoat.Main          → HTTP server on :8080
 *   java -cp ... com.cna.getgoat.Main --mcp    → MCP stdio server
 *
...
public class Main {
    ...
    private static String handleBranchPath(String path, HttpExchange exchange) throws IOException {
        ...
        if ("nodes".equals(parts[1]) && parts.length == 2 && "GET".equals(exchange.getRequestMethod()))
            return ctx.branchManager.exportFlatListJson(treeId);
    ...
    private static String handleDistance(String q) {
        double lat1 = qp(q,"lat1",Double.NaN), lng1 = qp(q,"lng1",Double.NaN);
        double lat2 = qp(q,"lat2",Double.NaN), lng2 = qp(q,"lng2",Double.NaN);
        if (Double.isNaN(lat1) || Double.isNaN(lng1) || Double.isNaN(lat2) || Double.isNaN(lng2))
            return "{\"error\":\"lat1,lng1,lat2,lng2 required\"}";
        double d = SphericalEngine.haversineDistance(lat1, lng1, lat2, lng2);
    ...
    private static String handleSimulate(String treeId, String nodeId) {
        if (treeId == null || nodeId == null) return "{\"error\":\"tree,node required\"}";
        var agent = new com.cna.getgoat.agent.Commander() {
            @Override public JsonNode callLLM(JsonNode m) { return null; }
        };
        agent.initialize(new com.cna.getgoat.config.CommanderConfig(), ctx.branchManager,
        ...
        if (grid != null) {
            ...
            for (int row = startRow; row <= endRow; row++) {
                ...
                for (int col = startCol; col <= endCol; col++) {
                    ...
                    double dist = com.cna.getgoat.map.geometry.SphericalEngine
    ...
    private static GeoBounds parseBounds(String q) {
        ...
        // Detect single-point query: lat + lng without bounds → tiny bbox
        double lat = qp(q, "lat", Double.NaN);
        double lng = qp(q, "lng", Double.NaN);
        if (!Double.isNaN(lat) && !Double.isNaN(lng)) {
            double d = 0.01; // ~1km box
            return new GeoBounds(lat - d, lat + d, lng - d, lng + d);
        }
