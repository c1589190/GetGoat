...
public interface AgentMode {
     ...
     *
     * Called by BaseAgent at the start of executeRound(). The returned messages
     * are appended directly to the LLM conversation (after the system prompt and
     * any past-round replay handled by BaseAgent).
     *
     * @param agent    the calling BaseAgent (provides access to managers)
     ...
     */
    JsonNode buildCurrentRoundContext(BaseAgent agent, BranchNode current,
     ...
     */
    String dispatchTool(BaseAgent agent, String toolName, JsonNode args);
     ...
     */
    default void onRoundComplete(BaseAgent agent, CommanderAction action) {}
     ...
     */
    default JsonNode buildSystemMessage(BaseAgent agent) {
     ...
     */
    default ArrayNode buildPastRoundReplay(BaseAgent agent, BranchNode curNode,
