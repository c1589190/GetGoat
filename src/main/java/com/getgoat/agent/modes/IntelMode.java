...
public class IntelMode implements AgentMode {
    ...
    private String searchWeb(BaseAgent agent, String query) {
        try {
    ...
    private String fetchUrl(BaseAgent agent, String url) {
        try {
            HttpClient client = HttpClient.newBuilder()
                .connectTimeout(Duration.ofSeconds(10)).build();
            HttpRequest req = HttpRequest.newBuilder()
    ...
    private String queryTerrain(BaseAgent agent, double lat, double lng) {
    ...
    private String queryRadius(BaseAgent agent, double lat, double lng, double r) {
    ...
    private String batchCreateUnits(BaseAgent agent, String unitsJson) {
        try {
    ...
    private String createBranchTree(BaseAgent agent, String name, String strategy) {
        var tree = agent.getBranchManager().createTree(name);
